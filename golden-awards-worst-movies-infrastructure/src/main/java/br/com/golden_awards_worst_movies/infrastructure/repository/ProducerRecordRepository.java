...
public interface ProducerRecordRepository extends JpaRepository<ProducerRecordEntity, Long> {
    ...
    @Query("SELECT pr FROM ProducerRecordEntity pr WHERE pr.producer.name = :producerName")
    Optional<ProducerRecordEntity> findByProducerName(@Param("producerName") String producerName);
